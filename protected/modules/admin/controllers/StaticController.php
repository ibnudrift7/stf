...
class StaticController extends ControllerAdmin
{
	...
	public function actionAdministrasi()
	{
		...
		$this->render('administrasi',array(
			'model'=>$model,
		));
	}

	public function actionFakta()
	{
		$model = Setting::model()->getModelSetting('data');

		$model = $this->loadData($model);

		$this->render('fakta',array(
			'model'=>$model,
		));
	}
