 ...
 */
class Infokampus extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return Infokampus the static model class
	 */
	...
	public function relations()
	{
		...
		return array(
			'description'=>array(self::HAS_ONE, 'InfokampusDescription', 'parents_id'),
		);
	}
	...
	public function search($language_id)
	{
		...
		";
		$criteria->addCondition('pg_infokampus_description.language_id = :language_id');
	...
	public function getData($id, $languageId=1)
	{
		...
		$model = Infokampus::model()->find($criteria);
	...
	public function getDataDesc($id, $languageId=1)
	{
		...
		$criteria->addCondition('parents_id = :id');
		$criteria->params = array(
			':language_id'=>$languageId,
			':id'=>$id,
		);
	...
	public function getAllData($limit = false, $id = false, $languageId=1)
	{
		...
		// if ($_GET['q'] != '') {
		// 	$criteria->addCondition('(pg_infokampus_description.content LIKE :q OR pg_infokampus_description.title LIKE :q)');
		// 	$params[':q'] = '%'.$_GET['q'].'%';
